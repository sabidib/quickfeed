...
def templated_response(request: Request, name: str, context: dict) -> HTMLResponse:
...
def feed_page_index(request: Request, page: T.Optional[int] = 1, per_page: T.Optional[int] = 15):
...
def feed_page(
    ...
    with request.app.state.session_maker() as session:
        ...
        article_models = [article for feed in article_feeds for article in feed.articles]
...
def delete_feed_page(request: Request,
...
def category_details(request: Request, category_id: str,
                     error: T.Optional[str] = None, success: T.Optional[str] = None):
    with request.app.state.session_maker() as session:
...
def delete_category_get(request: Request,
...
def bookmarks_page(request: Request, page: T.Optional[int] = 1, per_page: T.Optional[int] = 15):
    with request.app.state.session_maker() as session:
        bookmark_list = api.get_bookmark_list(session)
